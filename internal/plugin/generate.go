...
func Generate(plugin *protogen.Plugin, t transport.Transport) error {
	plugin.SupportedFeatures |= 1 // proto3 optional
	filesPerPackage, err := collectServices(plugin, t)
	if err != nil {
		return err
	}
	return generate(plugin, t, filesPerPackage)
}
...
func generate(plugin *protogen.Plugin, t transport.Transport, filesPerPackage map[protoreflect.FullName][]File) error {
	for _, files := range filesPerPackage {
		generateForPackage(plugin, t, files)
		for _, file := range files {
			f := createServiceTestFile(plugin, t, file)
			f.Skip()
...
func generateForPackage(plugin *protogen.Plugin, t transport.Transport, files []File) {
	outImportPath, outPkgName := t.OutputPackage(files[0].GoImportPath, files[0].GoPackageName)
	baseDir := filepath.Dir(files[0].GeneratedFilenamePrefix)
	if t.IsConnect() {
		baseDir = filepath.Join(baseDir, string(outPkgName))
	}
	filename := filepath.Join(baseDir, fileSuffix)
	f := plugin.NewGeneratedFile(filename, outImportPath)
	writeHeader(outPkgName, f)
...
func createServiceTestFile(plugin *protogen.Plugin, t transport.Transport, file File) *protogen.GeneratedFile {
	outImportPath, outPkgName := t.OutputPackage(file.GoImportPath, file.GoPackageName)
	prefix := file.GeneratedFilenamePrefix
	if t.IsConnect() {
		dir := filepath.Dir(prefix)
		base := filepath.Base(prefix)
		prefix = filepath.Join(dir, string(outPkgName), base)
	}
	filename := fmt.Sprintf("%s_%s", prefix, fileSuffix)
	f := plugin.NewGeneratedFile(filename, outImportPath)
	writeHeader(outPkgName, f)
	return f
}

func writeHeader(pkgName protogen.GoPackageName, f *protogen.GeneratedFile) {
