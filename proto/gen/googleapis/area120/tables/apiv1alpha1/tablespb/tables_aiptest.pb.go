...
func (fx *TablesServiceRowTestSuiteConfig) clone() *TablesServiceRowTestSuiteConfig {
	clone := *fx
	return &clone
}
...
func (fx *TablesServiceRowTestSuiteConfig) testList(t *testing.T) {
...

	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.Rows) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
// clone creates an isolated copy of the fixture for parallel test execution.
// This prevents race conditions on the currParent.
func (fx *TablesServiceTableTestSuiteConfig) clone() *TablesServiceTableTestSuiteConfig {
	clone := *fx
	return &clone
}
...
// clone creates an isolated copy of the fixture for parallel test execution.
// This prevents race conditions on the currParent.
func (fx *TablesServiceWorkspaceTestSuiteConfig) clone() *TablesServiceWorkspaceTestSuiteConfig {
	clone := *fx
	return &clone
}
