...
func (fx *ModelServiceModelTestSuiteConfig) testList(t *testing.T) {
...

	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.Models) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
func (fx *ModelServiceModelEvaluationTestSuiteConfig) testList(t *testing.T) {
...

	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.ModelEvaluations) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
func (fx *ModelServiceModelEvaluationSliceTestSuiteConfig) testList(t *testing.T) {
...

	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.ModelEvaluationSlices) > 0, "expected server to return at least 1 resource with page_size=0")
		})
