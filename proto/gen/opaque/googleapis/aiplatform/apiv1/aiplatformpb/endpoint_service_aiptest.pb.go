...
func (fx *EndpointServiceEndpointTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req.SetParent("")
		req.SetEndpoint(fx.Create(fx.nextParent(t, false)))
		_, err := fx.Service().CreateEndpoint(fx.Context(), req)
...
func (fx *EndpointServiceEndpointTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().GetEndpoint(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetEndpoint(fx.Context(), req)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetEndpointRequest{}
		req.SetName("projects/-/locations/-/endpoints/-")
		_, err := fx.Service().GetEndpoint(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *EndpointServiceEndpointTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetEndpoint(msg)
		_, err := fx.Service().UpdateEndpoint(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("persisted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		updateReq := &UpdateEndpointRequest{}
		updateReq.SetEndpoint(created)
		updated, err := fx.Service().UpdateEndpoint(fx.Context(), updateReq)
		assert.NilError(t, err)
		getReq := &GetEndpointRequest{}
		getReq.SetName(updated.GetName())
		persisted, err := fx.Service().GetEndpoint(fx.Context(), getReq)
		assert.NilError(t, err)
	...
	t.Run("preserve create_time", func(t *testing.T) {
		...
		req := &UpdateEndpointRequest{}
		req.SetEndpoint(created)
		req.SetUpdateMask(&fieldmaskpb.FieldMask{
			Paths: []string{
				"*",
			},
		})
		updated, err := fx.Service().UpdateEndpoint(fx.Context(), req)
		assert.NilError(t, err)
...
func (fx *EndpointServiceEndpointTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListEndpoints(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListEndpoints(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListEndpoints(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetEndpoints()))
				msgs = append(msgs, response.GetEndpoints()...)
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.GetEndpoints()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteEndpointRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteEndpoint(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListEndpointsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListEndpoints(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
...
func (fx *EndpointServiceEndpointTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteEndpoint(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteEndpointRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteEndpoint(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteEndpointRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteEndpoint(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
