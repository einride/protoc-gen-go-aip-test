...
func (fx *DatasetServiceAnnotationTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		_, err := fx.Service().ListAnnotations(fx.Context(), req)
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListAnnotations(fx.Context(), req)
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListAnnotations(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetAnnotations()))
				msgs = append(msgs, response.GetAnnotations()...)
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.GetAnnotations()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
func (fx *DatasetServiceAnnotationSpecTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetAnnotationSpecRequest{}
		req.SetName("")
		_, err := fx.Service().GetAnnotationSpec(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetAnnotationSpec(fx.Context(), req)
		assert.NilError(t, err)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetAnnotationSpecRequest{}
		req.SetName("projects/-/locations/-/datasets/-/annotationSpecs/-")
		_, err := fx.Service().GetAnnotationSpec(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *DatasetServiceDataItemTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &ListDataItemsRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListDataItems(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListDataItems(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetDataItems(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListDataItems(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetDataItems()))
				msgs = append(msgs, response.GetDataItems()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetDataItems()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
func (fx *DatasetServiceDatasetTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req.SetParent("")
		req.SetDataset(fx.Create(fx.nextParent(t, false)))
		_, err := fx.Service().CreateDataset(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *DatasetServiceDatasetTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().GetDataset(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		req := &GetDatasetRequest{}
		req.SetName(created.GetName())
		msg, err := fx.Service().GetDataset(fx.Context(), req)
		assert.NilError(t, err)
		assert.DeepEqual(t, msg, created, protocmp.Transform())
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetDatasetRequest{}
		req.SetName("projects/-/locations/-/datasets/-")
		_, err := fx.Service().GetDataset(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *DatasetServiceDatasetTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetDataset(msg)
		_, err := fx.Service().UpdateDataset(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("persisted", func(t *testing.T) {
		...
		updateReq.SetDataset(created)
		updated, err := fx.Service().UpdateDataset(fx.Context(), updateReq)
		assert.NilError(t, err)
		getReq := &GetDatasetRequest{}
		getReq.SetName(updated.GetName())
		persisted, err := fx.Service().GetDataset(fx.Context(), getReq)
		assert.NilError(t, err)
	...
	t.Run("preserve create_time", func(t *testing.T) {
		...
		req := &UpdateDatasetRequest{}
		req.SetDataset(created)
		req.SetUpdateMask(&fieldmaskpb.FieldMask{
			Paths: []string{
				"*",
			},
		})
		updated, err := fx.Service().UpdateDataset(fx.Context(), req)
		assert.NilError(t, err)
...
func (fx *DatasetServiceDatasetTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListDatasets(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListDatasets(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListDatasets(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetDatasets()))
				msgs = append(msgs, response.GetDatasets()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetDatasets()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteDatasetRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteDataset(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListDatasetsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListDatasets(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
...
func (fx *DatasetServiceDatasetTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteDataset(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteDatasetRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteDataset(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteDatasetRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteDataset(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
...
func (fx *DatasetServiceDatasetVersionTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req.SetParent("")
		req.SetDatasetVersion(fx.Create(fx.nextParent(t, false)))
		_, err := fx.Service().CreateDatasetVersion(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *DatasetServiceDatasetVersionTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetDatasetVersionRequest{}
		req.SetName("")
		_, err := fx.Service().GetDatasetVersion(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		req := &GetDatasetVersionRequest{}
		req.SetName(created.GetName())
		msg, err := fx.Service().GetDatasetVersion(fx.Context(), req)
		assert.NilError(t, err)
		assert.DeepEqual(t, msg, created, protocmp.Transform())
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetDatasetVersionRequest{}
		req.SetName("projects/-/locations/-/datasets/-/datasetVersions/-")
		_, err := fx.Service().GetDatasetVersion(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *DatasetServiceDatasetVersionTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &ListDatasetVersionsRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListDatasetVersions(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListDatasetVersions(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetDatasetVersions(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListDatasetVersions(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetDatasetVersions()))
				msgs = append(msgs, response.GetDatasetVersions()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetDatasetVersions()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			const deleteCount = 3
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteDatasetVersionRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteDatasetVersion(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListDatasetVersionsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListDatasetVersions(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs[deleteCount:],
...
func (fx *DatasetServiceDatasetVersionTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &DeleteDatasetVersionRequest{}
		req.SetName("")
		_, err := fx.Service().DeleteDatasetVersion(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteDatasetVersionRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteDatasetVersion(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteDatasetVersionRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteDatasetVersion(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
...
func (fx *DatasetServiceSavedQueryTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListSavedQueries(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListSavedQueries(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetSavedQueries(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListSavedQueries(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetSavedQueries()))
				msgs = append(msgs, response.GetSavedQueries()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetSavedQueries()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			const deleteCount = 3
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteSavedQueryRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteSavedQuery(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListSavedQueriesRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListSavedQueries(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs[deleteCount:],
...
func (fx *DatasetServiceSavedQueryTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &DeleteSavedQueryRequest{}
		req.SetName("")
		_, err := fx.Service().DeleteSavedQuery(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteSavedQueryRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteSavedQuery(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteSavedQueryRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteSavedQuery(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &DeleteSavedQueryRequest{}
		req.SetName("projects/-/locations/-/datasets/-/savedQueries/-")
		_, err := fx.Service().DeleteSavedQuery(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
