...
func (fx *IndexServiceIndexTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req.SetParent("")
		req.SetIndex(fx.Create(fx.nextParent(t, false)))
		_, err := fx.Service().CreateIndex(fx.Context(), req)
...
func (fx *IndexServiceIndexTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req := &GetIndexRequest{}
		req.SetName("")
		_, err := fx.Service().GetIndex(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetIndex(fx.Context(), req)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetIndexRequest{}
		req.SetName("projects/-/locations/-/indexes/-")
		_, err := fx.Service().GetIndex(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *IndexServiceIndexTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetIndex(msg)
		_, err := fx.Service().UpdateIndex(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("etag updated", func(t *testing.T) {
		...
		req := &UpdateIndexRequest{}
		req.SetIndex(msg)
		updated, err := fx.Service().UpdateIndex(fx.Context(), req)
		assert.NilError(t, err)
		_ = updated
	})
...
func (fx *IndexServiceIndexTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListIndexes(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListIndexes(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListIndexes(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetIndexes()))
				msgs = append(msgs, response.GetIndexes()...)
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.GetIndexes()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				...
				_, err := fx.Service().DeleteIndex(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListIndexesRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListIndexes(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
...
func (fx *IndexServiceIndexTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteIndex(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteIndexRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteIndex(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteIndexRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteIndex(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
