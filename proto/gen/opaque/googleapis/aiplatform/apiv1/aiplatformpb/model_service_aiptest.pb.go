...
func (fx *ModelServiceModelTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req := &GetModelRequest{}
		req.SetName("")
		_, err := fx.Service().GetModel(fx.Context(), req)
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetModel(fx.Context(), req)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetModelRequest{}
		req.SetName("projects/-/locations/-/models/-")
		_, err := fx.Service().GetModel(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *ModelServiceModelTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetModel(msg)
		_, err := fx.Service().UpdateModel(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("persisted", func(t *testing.T) {
		...
		updateReq.SetModel(created)
		updated, err := fx.Service().UpdateModel(fx.Context(), updateReq)
		assert.NilError(t, err)
		getReq := &GetModelRequest{}
		getReq.SetName(updated.GetName())
		persisted, err := fx.Service().GetModel(fx.Context(), getReq)
		assert.NilError(t, err)
	...
	t.Run("preserve create_time", func(t *testing.T) {
		...
		req := &UpdateModelRequest{}
		req.SetModel(created)
		req.SetUpdateMask(&fieldmaskpb.FieldMask{
			Paths: []string{
				"*",
			},
		})
		updated, err := fx.Service().UpdateModel(fx.Context(), req)
		assert.NilError(t, err)
...
func (fx *ModelServiceModelTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		_, err := fx.Service().ListModels(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListModels(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListModels(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetModels()))
				msgs = append(msgs, response.GetModels()...)
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.GetModels()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				...
				_, err := fx.Service().DeleteModel(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListModelsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListModels(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
...
func (fx *ModelServiceModelTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteModel(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteModelRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteModel(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteModelRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteModel(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
...
func (fx *ModelServiceModelEvaluationTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetModelEvaluationRequest{}
		req.SetName("")
		_, err := fx.Service().GetModelEvaluation(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		req := &GetModelEvaluationRequest{}
		req.SetName(created.GetName())
		msg, err := fx.Service().GetModelEvaluation(fx.Context(), req)
		assert.NilError(t, err)
		assert.DeepEqual(t, msg, created, protocmp.Transform())
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetModelEvaluationRequest{}
		req.SetName("projects/-/locations/-/models/-/evaluations/-")
		_, err := fx.Service().GetModelEvaluation(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *ModelServiceModelEvaluationTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &ListModelEvaluationsRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListModelEvaluations(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListModelEvaluations(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetModelEvaluations(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListModelEvaluations(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetModelEvaluations()))
				msgs = append(msgs, response.GetModelEvaluations()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetModelEvaluations()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
...
func (fx *ModelServiceModelEvaluationSliceTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetModelEvaluationSliceRequest{}
		req.SetName("")
		_, err := fx.Service().GetModelEvaluationSlice(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		req := &GetModelEvaluationSliceRequest{}
		req.SetName(created.GetName())
		msg, err := fx.Service().GetModelEvaluationSlice(fx.Context(), req)
		assert.NilError(t, err)
		assert.DeepEqual(t, msg, created, protocmp.Transform())
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetModelEvaluationSliceRequest{}
		req.SetName("projects/-/locations/-/models/-/evaluations/-/slices/-")
		_, err := fx.Service().GetModelEvaluationSlice(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *ModelServiceModelEvaluationSliceTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &ListModelEvaluationSlicesRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListModelEvaluationSlices(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListModelEvaluationSlices(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetModelEvaluationSlices(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListModelEvaluationSlices(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetModelEvaluationSlices()))
				msgs = append(msgs, response.GetModelEvaluationSlices()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetModelEvaluationSlices()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
