	...
	}
	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			for {
				...
				req.SetPageSize(0)
				req.SetPageToken(nextPageToken)
				page, err := fx.Service().ListModels(fx.Context(), req)
				assert.NilError(t, err)
				msgs = append(msgs, page.GetModels()...)
	...
	}
	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			for {
				...
				req.SetPageSize(0)
				req.SetPageToken(nextPageToken)
				page, err := fx.Service().ListModelEvaluations(fx.Context(), req)
				assert.NilError(t, err)
				msgs = append(msgs, page.GetModelEvaluations()...)
				nextPageToken = page.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
	...
	}
	{
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			for {
				...
				req.SetPageSize(0)
				req.SetPageToken(nextPageToken)
				page, err := fx.Service().ListModelEvaluationSlices(fx.Context(), req)
				assert.NilError(t, err)
				msgs = append(msgs, page.GetModelEvaluationSlices()...)
				nextPageToken = page.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
