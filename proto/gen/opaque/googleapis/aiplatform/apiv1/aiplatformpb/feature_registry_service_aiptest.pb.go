...
func (fx *FeatureRegistryServiceFeatureGroupTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req := &CreateFeatureGroupRequest{}
		req.SetParent("")
		req.SetFeatureGroup(fx.Create(fx.nextParent(t, false)))
		req.SetFeatureGroupId(userSetID)
		_, err := fx.Service().CreateFeatureGroup(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *FeatureRegistryServiceFeatureGroupTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetFeatureGroupRequest{}
		req.SetName("")
		_, err := fx.Service().GetFeatureGroup(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		req := &GetFeatureGroupRequest{}
		req.SetName(created.GetName())
		msg, err := fx.Service().GetFeatureGroup(fx.Context(), req)
		assert.NilError(t, err)
		assert.DeepEqual(t, msg, created, protocmp.Transform())
	})
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetFeatureGroupRequest{}
		req.SetName("projects/-/locations/-/featureGroups/-")
		_, err := fx.Service().GetFeatureGroup(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *FeatureRegistryServiceFeatureGroupTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		msg := fx.Update(parent)
		msg.SetName("")
		req := &UpdateFeatureGroupRequest{}
		req.SetFeatureGroup(msg)
		_, err := fx.Service().UpdateFeatureGroup(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("etag updated", func(t *testing.T) {
		...
		req := &UpdateFeatureGroupRequest{}
		req.SetFeatureGroup(msg)
		updated, err := fx.Service().UpdateFeatureGroup(fx.Context(), req)
		assert.NilError(t, err)
		_ = updated
	})
...
func (fx *FeatureRegistryServiceFeatureGroupTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &ListFeatureGroupsRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListFeatureGroups(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListFeatureGroups(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
				response.GetFeatureGroups(),
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListFeatureGroups(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetFeatureGroups()))
				msgs = append(msgs, response.GetFeatureGroups()...)
				nextPageToken = response.GetNextPageToken()
				if nextPageToken == "" {
					break
				}
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.NilError(t, err)
			// Server should use a default page size and return at least some results
			assert.Check(t, len(response.GetFeatureGroups()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			const deleteCount = 3
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteFeatureGroupRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteFeatureGroup(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListFeatureGroupsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListFeatureGroups(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs[deleteCount:],
...
func (fx *FeatureRegistryServiceFeatureGroupTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &DeleteFeatureGroupRequest{}
		req.SetName("")
		_, err := fx.Service().DeleteFeatureGroup(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteFeatureGroupRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteFeatureGroup(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteFeatureGroupRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteFeatureGroup(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
