...
func (fx *CloudSchedulerJobTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		...
		req.SetParent("")
		req.SetJob(fx.Create(fx.nextParent(t, false)))
		_, err := fx.Service().CreateJob(fx.Context(), req)
	...
	t.Run("persisted", func(t *testing.T) {
		...
		getReq := &CreateJobRequest{}
		getReq.SetParent(parent)
		getReq.SetJob(fx.Create(parent))
		msg, err := fx.Service().CreateJob(fx.Context(), getReq)
		assert.NilError(t, err)
		createReq := &GetJobRequest{}
		createReq.SetName(msg.GetName())
		persisted, err := fx.Service().GetJob(fx.Context(), createReq)
		assert.NilError(t, err)
	...
	t.Run("resource references", func(t *testing.T) {
		...
		t.Run(".pubsub_target.topic_name", func(t *testing.T) {
			...
			container.SetTopicName("invalid resource name")
			req := &CreateJobRequest{}
...
func (fx *CloudSchedulerJobTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req := &GetJobRequest{}
		req.SetName("")
		_, err := fx.Service().GetJob(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetJob(fx.Context(), req)
		assert.NilError(t, err)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetJobRequest{}
		req.SetName("projects/-/locations/-/jobs/-")
		_, err := fx.Service().GetJob(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *CloudSchedulerJobTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetJob(msg)
		_, err := fx.Service().UpdateJob(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("persisted", func(t *testing.T) {
		...
		updateReq.SetJob(created)
		updated, err := fx.Service().UpdateJob(fx.Context(), updateReq)
		assert.NilError(t, err)
		getReq := &GetJobRequest{}
		getReq.SetName(updated.GetName())
		persisted, err := fx.Service().GetJob(fx.Context(), getReq)
		assert.NilError(t, err)
...
func (fx *CloudSchedulerJobTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		...
		req := &ListJobsRequest{}
		req.SetParent("invalid resource name")
		_, err := fx.Service().ListJobs(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...

	{
		...
		t.Run("isolation", func(t *testing.T) {
			...
			req.SetParent(parent)
			req.SetPageSize(999)
			response, err := fx.Service().ListJobs(fx.Context(), req)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
				parentMsgs,
		...
		t.Run("one by one", func(t *testing.T) {
			...
			for {
				...
				response, err := fx.Service().ListJobs(fx.Context(), req)
				assert.NilError(t, err)
				assert.Equal(t, 1, len(response.GetJobs()))
				msgs = append(msgs, response.GetJobs()...)
		...
		t.Run("page size zero", func(t *testing.T) {
			...
			assert.Check(t, len(response.GetJobs()) > 0, "expected server to return at least 1 resource with page_size=0")
		})
	...
	}
	{
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				delReq := &DeleteJobRequest{}
				delReq.SetName(parentMsgs[i].GetName())
				_, err := fx.Service().DeleteJob(fx.Context(), delReq)
				assert.NilError(t, err)
			}
			listReq := &ListJobsRequest{}
			listReq.SetParent(parent)
			listReq.SetPageSize(9999)
			response, err := fx.Service().ListJobs(fx.Context(), listReq)
			assert.NilError(t, err)
			assert.DeepEqual(
				t,
...
func (fx *CloudSchedulerJobTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteJob(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteJobRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteJob(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteJobRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteJob(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
...
func (fx *CloudSchedulerJobTestSuiteConfig) create(t *testing.T, parent string) *Job {
	...
	req := &CreateJobRequest{}
	req.SetParent(parent)
	req.SetJob(fx.Create(parent))
	created, err := fx.Service().CreateJob(fx.Context(), req)
	assert.NilError(t, err)
	return created
}
