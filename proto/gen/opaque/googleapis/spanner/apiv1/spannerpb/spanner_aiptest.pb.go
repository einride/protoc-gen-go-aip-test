...
func (fx *SpannerSessionTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().GetSession(fx.Context(), req)
	...
	t.Run("exists", func(t *testing.T) {
		...
		msg, err := fx.Service().GetSession(fx.Context(), req)
	...
	t.Run("only wildcards", func(t *testing.T) {
		fx.maybeSkip(t)
		req := &GetSessionRequest{}
		req.SetName("projects/-/instances/-/databases/-/sessions/-")
		_, err := fx.Service().GetSession(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
...
func (fx *SpannerSessionTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		req.SetName("")
		_, err := fx.Service().DeleteSession(fx.Context(), req)
		assert.Equal(t, codes.InvalidArgument, status.Code(err), err)
	})
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		req1 := &DeleteSessionRequest{}
		req1.SetName(created.GetName())
		deleted, err := fx.Service().DeleteSession(fx.Context(), req1)
		assert.NilError(t, err)
		_ = deleted
		req2 := &DeleteSessionRequest{}
		req2.SetName(created.GetName())
		_, err = fx.Service().DeleteSession(fx.Context(), req2)
		assert.Equal(t, codes.NotFound, status.Code(err), err)
	})
